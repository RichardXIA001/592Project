...
import rocketland_scripts.rocketland as rocketland
...
dataset = rocketland.LunarLanding(numpoints=65000, pretrain=opt.pretrain, tMin=opt.tMin,
...
def val_fn(model, ckpt_dir, epoch):
  ...
  for i in range(num_times):
    ...
    #   param_values = torch.ones(mgrid_coords.shape[0], 1) * (slices_toplot[j]['param'] - beta['param']) / alpha['param']
      coords = torch.cat((time_coords, mgrid_coords, th, y_dot, z_dot, th_dot), dim=1) 
