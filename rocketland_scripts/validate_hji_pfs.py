...
import json
from utils import estimate_failure_probability
...
for i in range(num_times):
  ...
  for j in range(num_slices):
    ...
    model_out = estimate_failure_probability(model_out)
    
    # estimate failure probability
    
    print("min, max of model_out: ", np.min(model_out), np.max(model_out))
...
fig.savefig(os.path.join(directory, 'Pfs_validation_plot_epoch_%04d.png' % opt.checkpoint_toload))

print('save Pfs Plot to %s' % os.path.join(directory, 'Pfs_validation_plot_epoch_%04d.png' % opt.checkpoint_toload))
